"""GoldBot Core Owner Monitoring Alpha, TASK 7 -- telegram/owner/monitoring_commands.py tests."""
...
def test_get_status_report_shows_na_for_missing_last_scan():
    with patch("telegram.owner.monitoring_commands.get_system_health_snapshot", return_value=_system_health(last_scan=None)):
        result = get_status_report()
    assert "N/A" in result.message


def test_get_status_report_includes_last_error_when_present():
    with patch("telegram.owner.monitoring_commands.get_system_health_snapshot", return_value=_system_health(last_error="boom")):
        result = get_status_report()
    assert "boom" in result.message


def test_get_status_report_never_raises_on_failure():
    with patch("telegram.owner.monitoring_commands.get_system_health_snapshot", side_effect=Exception("fail")):
        result = get_status_report()
...
def test_get_health_report_delegates_to_system_commands():
    ...
    with patch("telegram.owner.monitoring_commands.get_system_health", return_value=expected) as mocked:
...
def test_get_market_report_returns_provider_command_result():
    with patch("telegram.owner.monitoring_commands.get_market_health") as mocked:
...
def test_get_market_report_never_raises_on_failure():
    with patch("telegram.owner.monitoring_commands.get_market_health", side_effect=Exception("fail")):
        result = get_market_report()
    assert result.success is False
...
def test_get_signals_report_shows_counts():
    ...
    with patch("telegram.owner.monitoring_commands.get_signal_health", return_value=health):
...
def test_get_signals_report_never_raises_on_failure():
    with patch("telegram.owner.monitoring_commands.get_signal_health", side_effect=Exception("fail")):
        result = get_signals_report()
    assert result.success is False
...
def test_get_daily_report_composes_system_signals_and_errors():
    ...
    with patch("telegram.owner.monitoring_commands.get_system_health_snapshot", return_value=_system_health()), \
         patch("telegram.owner.monitoring_commands.get_signal_health", return_value=signals), \
