...
# ---------------------------------------------------------------------------
# V2 Phase 6.1 (Director Approved) -- nav_back/nav_home (Director Decision
# 4). Both currently resolve to the same destination (route_command("/start",
# ...)), edited in place via the same edit-then-fallback shape _handle_
# language() already uses. Uses the real command_router (not mocked), same
# posture as the Wizard-advance tests above -- the point is to verify real
# cross-module wiring.
# ---------------------------------------------------------------------------


def test_nav_home_edits_the_message_in_place():
    repo = UserRepository()
    repo.create_user("901", username="navhomeuser")
    repo.complete_registration("901")

    callback = FakeCallback("nav_home", telegram_id="901")
    asyncio.run(route_callback(callback))

    assert callback.message.edited_with is not None
    assert callback.message.answered_with is None
    assert callback.answered is True


def test_nav_back_also_resolves_to_start():
    repo = UserRepository()
    repo.create_user("902", username="navbackuser")
    repo.complete_registration("902")

    home_callback = FakeCallback("nav_home", telegram_id="902")
    asyncio.run(route_callback(home_callback))

    back_callback = FakeCallback("nav_back", telegram_id="902")
    asyncio.run(route_callback(back_callback))

    assert back_callback.message.edited_with == home_callback.message.edited_with


def test_nav_home_falls_back_to_new_message_on_edit_failure():
    repo = UserRepository()
    repo.create_user("903", username="navfallbackuser")
    repo.complete_registration("903")

    callback = FakeCallback("nav_home", telegram_id="903")
    callback.message.edit_should_fail = True
    asyncio.run(route_callback(callback))
