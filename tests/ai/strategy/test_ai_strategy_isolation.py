"""
Phase 66.6 TASK 9 — ai/strategy/ stays isolated from every Trading
...
`tests/ai/performance/test_ai_performance_isolation.py`'s own AST-based
pattern exactly.
"""
...
def test_strategy_never_imports_assistant_voice():
...
def test_strategy_never_imports_core_package():
...
def test_strategy_never_imports_monitoring():
...
def test_strategy_never_imports_sql_or_persistence_libraries():
...
def test_strategy_never_imports_analytics_or_the_top_level_learning_package():
    """docs/PHASE66_6_AUDIT.md Question 2: analytics/ reviewed but not reused this phase; the pre-existing top-level learning/ package is likewise never imported."""
    for py_file in _strategy_dir().rglob("*.py"):
        for name in _imported_names(py_file):
...
def test_performance_import_confined_to_performance_adapter():
    """TASK 4: ai.performance is only permitted in performance_adapter.py -- models.py, access.py, strategy_runtime.py, journal_adapter.py, memory_adapter.py never import it."""
...
def test_trade_journal_import_confined_to_journal_adapter():
    """TASK 5: ai.trade_journal is only permitted in journal_adapter.py -- models.py, access.py, strategy_runtime.py, performance_adapter.py, memory_adapter.py never import it."""
