"""Phase 61.2 TASK 3 — Real Gemini Provider. No real network call anywhere -- every test injects a fake session."""

import requests
import pytest

from ai_layer.ai_engine.capabilities.capability import Capability
from ai_layer.ai_engine.providers.gemini_provider import GeminiProvider
...
class _FakeSecrets:
    ...
    def GEMINI_API_KEY(self):
        if self._api_key is None:
            raise ValueError("Secret 'GEMINI_API_KEY' not found in environment.")
...
def _gemini_success_payload(text="Gold is in an uptrend."):
    return {"candidates": [{"content": {"parts": [{"text": text}]}}]}


def test_health_check_false_without_api_key():
    provider = GeminiProvider(secrets=_FakeSecrets(api_key=None))
    assert provider.health_check() is False


def test_health_check_true_with_api_key():
    provider = GeminiProvider(secrets=_FakeSecrets(api_key="fake-key-123"))
    assert provider.health_check() is True


def test_name_is_gemini():
    provider = GeminiProvider(secrets=_FakeSecrets(api_key="fake-key"))
...
def test_analyze_returns_provider_result_on_success():
    session = _FakeSession(response=_FakeResponse(200, _gemini_success_payload("uptrend")))
    provider = GeminiProvider(session=session, secrets=_FakeSecrets(api_key="fake-key"))
...
def test_chat_and_explain_also_use_the_real_call_path():
    session = _FakeSession(response=_FakeResponse(200, _gemini_success_payload("hello")))
    provider = GeminiProvider(session=session, secrets=_FakeSecrets(api_key="fake-key"))
...
def test_api_key_travels_only_in_the_header_never_in_the_url():
    session = _FakeSession(response=_FakeResponse(200, _gemini_success_payload()))
    provider = GeminiProvider(session=session, secrets=_FakeSecrets(api_key="super-secret-key"))
...
def test_missing_api_key_error_message_does_not_leak_a_key():
    ...
    try:
        ...
        assert "GEMINI_API_KEY not configured" in str(e)
