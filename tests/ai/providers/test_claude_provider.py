"""Phase 61.5 TASK 1 — Real Claude Provider. No real network call anywhere -- every test injects a fake session."""

import requests
import pytest

from ai_layer.ai_engine.capabilities.capability import Capability
from ai_layer.ai_engine.providers.claude_provider import ClaudeProvider
...
def _claude_success_payload(text="Gold is in an uptrend."):
    return {"content": [{"text": text}]}


def test_health_check_false_without_api_key():
    provider = ClaudeProvider(secrets=_FakeSecrets(api_key=None))
    assert provider.health_check() is False


def test_health_check_true_with_api_key():
    provider = ClaudeProvider(secrets=_FakeSecrets(api_key="fake-key-123"))
    assert provider.health_check() is True


def test_name_is_claude():
    provider = ClaudeProvider(secrets=_FakeSecrets(api_key="fake-key"))
...
def test_analyze_returns_provider_result_on_success():
    session = _FakeSession(response=_FakeResponse(200, _claude_success_payload("uptrend")))
    provider = ClaudeProvider(session=session, secrets=_FakeSecrets(api_key="fake-key"))
...
def test_chat_and_explain_also_use_the_real_call_path():
    session = _FakeSession(response=_FakeResponse(200, _claude_success_payload("hello")))
    provider = ClaudeProvider(session=session, secrets=_FakeSecrets(api_key="fake-key"))
...
def test_api_key_travels_only_in_the_header_never_in_the_url():
    session = _FakeSession(response=_FakeResponse(200, _claude_success_payload()))
    provider = ClaudeProvider(session=session, secrets=_FakeSecrets(api_key="super-secret-key"))

    provider.analyze("prompt")

    call = session.calls[0]
    assert "super-secret-key" not in call["url"]
    assert call["headers"]["x-api-key"] == "super-secret-key"
...
def test_missing_api_key_error_message_does_not_leak_a_key():
    ...
    try:
        ...
        assert "CLAUDE_API_KEY not configured" in str(e)
