...
`media`/`broadcast`/`ai_layer.personal_ai.interaction_manager`/`ai_layer.ai_service.session` are allowed, but
confined to `ai_layer/voice_ai/adapter.py` (type-only reads) and, as of Phase
65.2, `ai_layer/voice_ai/conversation_bridge.py` (the one composition-root file
...
"Voice -> Knowledge (X), Voice -> Reasoning (X)" — `ai_layer.knowledge_ai.memory_manager`,
`ai_layer.ai_engine.reasoning`, `ai_layer.explanation_ai`, and top-level `knowledge` remain
...
def test_voice_package_never_imports_trading_translation_or_upstream_intelligence_layers():
    forbidden_prefixes = (
        ...
        "media_layer.translation", "ai_layer.knowledge_ai.memory_manager", "ai_layer.ai_engine.reasoning", "ai_layer.explanation_ai", "knowledge",
    )
...
def test_voice_package_never_imports_speech_or_llm_sdks():
    ...
    forbidden_import_prefixes = ("whisper", "speech_recognition", "pyaudio", "ai_layer.ai_engine.runtime")
...
def test_voice_package_media_broadcast_session_conversation_imports_confined_to_two_files():
    """Phase 65.1 TASK 9 + Phase 65.2 TASK 4 regression guard: only ai_layer/voice_ai/adapter.py (type-only reads) and ai_layer/voice_ai/conversation_bridge.py (the real ConversationEngine call) may import media/broadcast/ai.session/ai.conversation -- every other voice/*.py file stays exactly as isolated as Phase 65.0 left it."""
    allowed_prefixes = ("media_layer.content_manager", "media_layer.telegram_broadcast", "ai_layer.ai_service.session", "ai_layer.personal_ai.interaction_manager")
...
def test_conversation_engine_real_call_confined_to_conversation_bridge():
    """The strictest guard: the real, LLM-backed ai.conversation.conversation_engine module is imported by exactly one file in this package -- ai_layer/voice_ai/adapter.py only ever reads ai.conversation.models/ai.session.conversation_state (type-only, never the engine itself)."""
