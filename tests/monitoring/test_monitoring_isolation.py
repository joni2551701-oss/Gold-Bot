...
`decision_layer/decision_logger/decision_logger.py` never imports `signals`/`context`
...
def test_monitoring_never_imports_decision_risk_execution():
    # Layer-qualified after the Phase C migration. DecisionLogger is an
    # observability component and moved to its canonical home
    # decision_layer/decision_logger/ -- monitoring has always been allowed to
    # use it. What stays forbidden is decision/risk/execution *logic*.
    forbidden_prefixes = (
        "decision_layer.decision_engine",
        "decision_layer.rule_engine",
        "decision_layer.decision_service",
        "risk_layer",
        "execution_layer",
    )
...
def test_monitoring_models_are_primitive_only():
    """Belt-and-suspenders: core_layer/health_monitor/models.py itself imports no Trading Core or upstream-object modules at all -- only stdlib."""
