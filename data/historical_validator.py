...
write to the database, does not change data/historical_data_collector.py's
behavior.

Relationship to data/data_quality.py (Phase A8, read but not modified
here): that module's assess_data_quality() runs on
data.twelve_data_client.Candle (the live pipeline's in-memory,
...
data/data_quality.py's own docstring already established for
...
data.data_quality.INTERVAL_DELTAS -- a public, same-package module
...
def validate_historical_candles(
    ...
    data.data_quality.assess_data_quality()).
    """
    if not candles:
