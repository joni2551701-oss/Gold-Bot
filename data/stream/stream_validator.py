...
Checks each canonical `data.stream.stream_event.StreamEvent` (a price
...
its correct canonical layer, the candle level (`data.market_data`'s
`_validate_and_clean` and `data.data_quality.assess_data_quality`).
