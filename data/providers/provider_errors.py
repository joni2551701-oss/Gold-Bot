...
data/providers/ standardises on, so a caller catches one family of
...
2. Can an existing hierarchy be extended? ai/providers/runtime_errors.py
   ...
   data/providers/). Reusing it would couple the market-data layer to
...
def provider_error_for_status(
    ...
    data/api_error_classifier.py's non-raising style); the caller
