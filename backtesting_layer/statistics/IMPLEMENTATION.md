...
`ai_layer.knowledge_ai.learning_loop.pattern_detector.detect_patterns()` directly, picking the
...
`strategy_report.py` imports `backtesting_layer.statistics.signal_performance` (same
...
`backtesting_layer.statistics.strategy_report.compute_win_rate` at module level, plus
`backtesting_layer.statistics.equity_curve.max_drawdown` locally (inside
`compute_performance_metrics()`, only when a curve is supplied).
`equity_curve.py` imports `backtesting_layer.statistics.signal_performance`
...
`ai_layer.knowledge_ai.learning_loop.models`/`ai_layer.knowledge_ai.learning_loop.pattern_detector` (real, runtime imports
