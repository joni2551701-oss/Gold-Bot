...
Reuses `ai_layer.knowledge_ai.learning_loop.pattern_detector.detect_patterns()` directly, not
...
under Best Condition -- an HTF-alignment detail `ai_layer.knowledge_ai.learning_loop.models.LearningRecord`
does not carry as a structured field (only `strategy_name`/`session`/
`market_phase` are real grouping dimensions, same three
`ai_layer.knowledge_ai.learning_loop.pattern_detector.detect_patterns()` already groups by). This
...
from ai_layer.knowledge_ai.learning_loop.pattern_detector import PatternInsight, detect_patterns
