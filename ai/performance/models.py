...
`academy/`, `portfolio/`, `research/`, `core.`, or `ai.memory`.
...
name already exists at `analytics.performance_metrics.PerformanceMetrics`
...
lifecycle field to archive *onto*. Mirrors `ai.learning.models.LearningRecord`'s
...
class PerformanceRecord:
    ...
    trade_id: the originating trade's own identity -- caller-supplied,
        ...
        `ai.trade_journal.models.TradeJournalEntry.trade_id` already
        established).
    journal_id: an optional reference to the originating
        `ai.trade_journal.models.TradeJournalEntry.journal_id` -- a
    ...
    result: free-text trade outcome (e.g. "WIN"/"LOSS"/"BE"), no fixed
        ...
        posture `ai.trade_journal.models.TradeJournalEntry.result`
        already uses.
    ...
    archived: this record's own lifecycle flag, mutated only via
        ...
        `ai.coaching.models.CoachingRecommendation.status`'s own
