...
reuse**. `analytics.strategy_report.compute_win_rate()` -- a plain
`(wins, losses) -> float` utility already reused by
`analytics/performance_metrics.py` itself -- fits directly over
...
def performance_records_to_win_rate_metric(
    ...
    """
    Reuses `analytics.strategy_report.compute_win_rate()` directly --
    counts `PerformanceRecord.result == "WIN"`/`"LOSS"` (the same
    free-text vocabulary `ai.trade_journal.models.TradeJournalEntry.result`
