...
here (mirrors `ai/learning/models.py`'s own `level` field posture).
"""
...
class CoachingTopic(Enum):
    ...
    package. Mirrors `ai.learning.models.LearningTopic`'s own value
    set for coherence across the Learning -> Coaching pipeline stage,
    but is a separate, local enum (not imported from `ai.learning`) --
...
class CoachingStatus(Enum):
    ...
    `ai.learning.learning_runtime.LearningRuntime.archive()`'s own
...
class CoachingRecommendation:
    ...
    `ai.trade_journal.models.TradeJournalEntry` already used.
    ...
    learning_id: an optional reference to the originating
        `ai.learning.models.LearningRecord.id`, when this
    ...
    journal_id: an optional reference to the originating
        `ai.trade_journal.models.TradeJournalEntry.journal_id`, when
    ...
    recommendation: an optional, short free-text suggested action --
        ...
        `ai.trade_journal.models.TradeJournalEntry.reason` and
