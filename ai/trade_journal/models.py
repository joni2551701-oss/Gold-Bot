...
class TradeJournalEntry:
    ...
    journal_id: this entry's own generated identity
        (`generate_journal_id()`).
    chart_id: the originating `ai.chart_intelligence.models.ChartAnalysis.chart_id`
    ...
    result: free-text trade outcome (e.g. "WIN"/"LOSS"/"BE"), no fixed
        ...
        posture `ai.journal.failure_analysis.FailureAnalysisEntry.reason`
        and `learning.models.LearningRecord.result` both already use.
...
class ReplayContext:
    ...
    `ai.chart_intelligence.models.ChartContext.image_hash` already
    established.
