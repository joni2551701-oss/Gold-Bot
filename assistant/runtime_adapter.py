...
`voice.runtime`/`voice.models`. Every other file in this package keeps
...
`assistant.access.is_personal_ai_enabled_for()` itself (defense in
...
from ai.conversation.conversation_engine import ConversationEngine, ConversationResult
from ai.intelligence_runtime import IntelligenceRuntime, PipelineRun
...
def advance_conversation(
    ...
    `voice/conversation_bridge.py`'s own session-linking behavior.
    """
