...
class MarketHealth:
    ...
    latency: relayed from `core_layer.health_monitor.provider_health.ProviderHealthReport.latency_ms`.
    data_source_status: relayed from
        `core_layer.health_monitor.provider_health.ProviderHealthStatus.value`.
    """
...
class SignalHealth:
    ...
    total_signals/buy_count/sell_count/none_count: today's activity
        counts, aggregated from `database_layer.trade_repository.signal_repository.SignalRepository`
...
class ErrorEvent:
    ...
    `database_layer.audit_log.monitoring_repository.MonitoringRepository` (TASK 9) --
...
class ResourceSnapshot:
    ...
    restart_count: how many times this process has started, persisted
        via `database_layer.audit_log.monitoring_repository.MonitoringRepository`
    ...
    uptime_seconds: relayed from the already-existing
        `core_layer.health_monitor.system_monitor.SystemMonitor.uptime_seconds()` --
...
class PerformanceCounters:
    ...
    kind (that is `core_layer.health_monitor.performance.PerformanceTracker`'s own,
    separate, already-existing concern). Every count starts at 0 and
    is incremented one at a time by `core_layer.health_monitor.performance_collector.PerformanceCollector`.
    """
