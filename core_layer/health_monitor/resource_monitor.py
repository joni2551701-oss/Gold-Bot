...
observation, distinct from `core_layer/health_monitor/system_monitor.py`'s own
...
`core_layer.health_monitor.system_monitor.SystemMonitor.record_scan()`/`last_scan`
mechanism -- not duplicated here. "Latency" is likewise already
covered by `core_layer/health_monitor/provider_health.py` and `MarketHealth.latency_ms`
