...
logger only. Persisted via `database_layer.audit_log.monitoring_repository.MonitoringRepository`
(TASK 9), so `/errors`' own "Last 24h" window survives a process
restart.

`capture()` also relays the message into
`core_layer.health_monitor.system_monitor.DEFAULT_MONITOR.record_error()`, so
...
from database_layer.audit_log.monitoring_repository import MonitoringRepository
from core_layer.health_monitor.models import ErrorEvent, ErrorSeverity
from core_layer.health_monitor.system_monitor import record_error as record_system_error
