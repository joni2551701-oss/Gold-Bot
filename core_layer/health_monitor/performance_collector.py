...
of any kind (that is `core_layer.health_monitor.performance.PerformanceTracker`'s
...
`core_layer.health_monitor.system_monitor.SystemMonitor`/`DEFAULT_MONITOR` already
established -- counts reset on process restart by design (a
count-since-this-process-started concern, not a persisted history;
`core_layer.health_monitor.resource_monitor.get_resource_snapshot()`'s own
