...
pieces (no new health/status/provider logic is written here):
    platform_layer.telegram.owner.status_commands.get_system_status()      -- system/pipeline/database/provider/mode/last-signal
    platform_layer.telegram.owner.control_commands.get_feature_states()     -- runtime feature ON/OFF count
    platform_layer.telegram.owner.provider_commands.list_providers()        -- every registered provider's availability
...
already-existing service calls only:
    ...
    platform_layer.telegram.owner.ai_commands.ai_runtime_online()/current_provider_for()  (Phase 61.5 Addendum)
    platform_layer.telegram.owner.ai_commands.ai_cost()                                   (Phase 61.4 TASK 6)
...
from platform_layer.telegram.owner.ai_commands import ai_cost, ai_runtime_online, current_provider_for
from platform_layer.telegram.owner.control_commands import get_feature_states
from platform_layer.telegram.owner.provider_commands import ProviderCommandResult, list_providers
from platform_layer.telegram.owner.status_commands import get_system_status
