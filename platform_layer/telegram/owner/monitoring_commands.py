...
wired in `platform_layer/telegram/handlers.py`" posture as every other module in
`platform_layer/telegram/owner/` -- see `platform_layer/telegram/owner/provider_commands.py`'s own
docstring for the established convention.

`/health` reuses `platform_layer.telegram.owner.system_commands.get_system_health()`
...
`platform_layer/telegram/commands.py`'s `OWNER_COMMANDS` registration + the existing,
live `platform_layer.telegram.permissions`/`platform_layer.telegram.command_router` gate (see the
...
from platform_layer.telegram.owner.system_commands import get_system_health
...
def get_health_report() -> ProviderCommandResult:
    """/health -> platform_layer.telegram.owner.system_commands.get_system_health() (reused as-is, per the audit)."""
