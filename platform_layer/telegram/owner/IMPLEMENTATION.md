...
into `platform_layer/telegram/commands.py`'s `OWNER_COMMANDS`/`ADMIN_COMMANDS` dicts,
and NOT called from `platform_layer/telegram/command_router.py` or
`platform_layer/telegram/handlers.py`. The running Telegram bot's actual command
...
permission wiring (via the pre-existing `platform_layer/telegram/permissions.py`/
`platform_layer/telegram/command_router.py` pattern), and a handler that calls these
...
`platform_layer.telegram.admin_service.AdminService.get_system_status()` (reused, not
...
Dashboard — never imports or modifies `platform_layer.telegram.permissions.PermissionLevel`
(the live enum gating real commands today via
`platform_layer/telegram/command_router.py`). See `docs/OWNER_PERMISSIONS.md` for the
...
composing `platform_layer.telegram.admin_service.AdminService.get_system_status()`,
...
imports `platform_layer.telegram.admin_service.AdminService` and
...
`platform_layer.telegram.admin_service.AdminService`, and
