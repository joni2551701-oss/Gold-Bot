...
platform_layer/telegram/polling.py. This module turns (command text, Telegram user
...
from platform_layer.telegram.commands import COMMANDS, OWNER_COMMANDS, ADMIN_COMMANDS
...
from platform_layer.telegram import reply_keyboard_manager
...
# with an optional "current value" accessor (platform_layer.telegram.handlers'
...
def _start_keyboard(telegram_id, language):
    ...
    by this fix. is_owner()/is_admin() (platform_layer/telegram/permissions.py) are
...
class RouterResult:
    ...
    # every reply is now a new message; see platform_layer/telegram/polling.py's
...
async def route_command(command_text: str, telegram_id=None, username=None) -> RouterResult:
    ...
    Mapping (platform_layer.telegram.keyboards.resolve_navigation_command()) translates
