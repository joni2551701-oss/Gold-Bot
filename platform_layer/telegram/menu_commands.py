...
Registration happens once, at bot startup (platform_layer/telegram/polling.py's
...
to those live would require hooking into platform_layer/telegram/command_router.py,
platform_layer/telegram/handlers.py, or platform_layer/telegram/callback_router.py's language-change
...
platform_layer/telegram/polling.py's run_polling() from starting (same posture as
...
# docstring precedent (platform_layer.telegram.keyboards.admin_panel_keyboard()).
...
async def register_all_menus(bot: Bot) -> None:
    ...
    platform_layer/telegram/polling.py's run_polling(). Never raises.
    """
    try:
