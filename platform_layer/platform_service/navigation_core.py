...
(`platform_layer/platform_service/menu_registry.py`): `UI -> Platform Adapter -> Navigation
...
`platform_layer/telegram/permissions.py`'s own OWNER > ADMIN > USER rank comparison
by convention (same relationship `platforms/`'s `permission: str`
fields already have to `PermissionLevel` -- see
`platform_layer/platform_service/navigation_model.py`'s docstring) -- not a duplicate, since
`platform_layer/telegram/permissions.py`'s version resolves a Telegram user's tier
...
`platform_layer/telegram/reply_keyboard_manager.py`'s own `_LAST_SECTION` precedent
...
`platform_layer/telegram/reply_keyboard_manager.py`'s `keyboard_for_command()`/
...
def has_sufficient_permission(user_tier: str, required_tier: str) -> bool:
    ...
    `platform_layer/telegram/permissions.py`'s own fail-closed convention for an
    unresolved OWNER_ID.
    """
