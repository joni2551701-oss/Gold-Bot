...
not replace or modify `platform_layer/telegram/commands.py`'s live `COMMANDS`/
`ADMIN_COMMANDS`/`OWNER_COMMANDS` registries, which remain the single
source of truth `platform_layer/telegram/command_router.py` actually dispatches from
...
Foundation Reuse Audit (Constitution Article 11): `platform_layer/telegram/commands.py`
...
class MenuDefinition:
    ...
    Identity convention (`platform_layer.platform_service.navigation_model.is_valid_screen_id()`)
...
# docs/commands_reference.md and platform_layer/telegram/reply_keyboard_manager.py's
...
DEFAULT_MENUS: Tuple[MenuDefinition, ...] = (
    ...
    # platform_layer/telegram/reply_keyboard_manager.py's own Director Review correction 1.
...
def build_default_menu_registry() -> MenuRegistry:
    """Returns a fresh MenuRegistry seeded with DEFAULT_MENUS -- a read-only mirror of GoldBot's real, live Telegram screens, not a replacement for platform_layer/telegram/commands.py."""
