...
Foundation only, same posture `telegram/owner/provider_commands.py`
originally established (Phase 59.3 TASK 5): real, tested,
standalone functions -- NOT registered into `telegram/commands.py`'s
`OWNER_COMMANDS`, NOT wired into `telegram/command_router.py`'s
routing or `telegram/handlers.py` (Rule 7 -- "Owner Commandlar
...
def broadcast_enable(provider_name: str) -> BroadcastCommandResult:
    """/broadcast_enable PROVIDER -- always NOT IMPLEMENTED this phase. Never actually enables anything in broadcast/provider_manager.py."""
    return BroadcastCommandResult(success=False, message=_NOT_IMPLEMENTED)


def broadcast_disable(provider_name: str) -> BroadcastCommandResult:
    """/broadcast_disable PROVIDER -- always NOT IMPLEMENTED this phase. Never actually disables anything in broadcast/provider_manager.py."""
    return BroadcastCommandResult(success=False, message=_NOT_IMPLEMENTED)
