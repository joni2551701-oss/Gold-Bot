...
Phase 63.0 (Senior Trading AI Foundation), TASK 4. Foundation only.
...
- `models.py` — `BroadcastProviderType` (YOUTUBE/OBS/RTMP/TWITCH/KICK/
  ...
  `BroadcastProviderDescriptor`, `BroadcastTrigger`, `BroadcastRequest`.
- `provider_manager.py` — `BroadcastProviderManager`, a static catalog
  of the six provider types plus Owner-set ENABLED/DISABLED intent.
...
- `broadcast_manager.py` — `BroadcastManager`, composes the two above
  ...
  `BroadcastRequest` value — it never sends one anywhere.
