...
4. Not wired into `core/pipeline.py` in this phase — pipeline logic is
   unchanged.
...
| Decision | `decision`, `decision_score` | `decision` defaults `"PENDING"`; see "Decision status mapping" below. |
...
Minimal — not wired into `core/pipeline.py` in this phase:
...
and `strategies/` are all untouched by this phase. A future,
separately-approved phase would be the one to actually call
`from_signal_candidate()` inside `core/pipeline.py` and decide what to
do with the result.
