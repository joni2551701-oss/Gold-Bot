...
`decision/decision_engine.py`'s `DecisionEngine.evaluate()` is the
...
A strategy (`strategies/liquidity_strategy.py`, `fvg_strategy.py`,
...
used throughout this codebase (`signals/models.py`'s
...
`risk/risk_manager.py`'s `RiskManager.evaluate()` takes a
