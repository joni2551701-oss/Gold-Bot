...
| Voice Runtime | `voice/runtime.py` | ✅ `VoiceRuntime` already exists (Phase 65.0, extended 65.1), LOCKed — `generate_audio()`/`generate_with_fallback()` already do exactly the TTS half of this phase's TASK 6 flow | Reused as-is, called (not duplicated) from the new composition-root function this phase adds |
...
## The one new composition-root file: `voice/conversation_bridge.py`
...
`voice/adapter.py`'s existing four functions already respect (each
reads one upstream type's already-public fields, never calls into that
layer's engine). That boundary is correct for `voice/adapter.py`
...
(Phase 65.0/65.1) never imports `ai.conversation.conversation_engine`,
only `ai.conversation.models`/`ai.session.conversation_state`
(type-only). `ai/intelligence_runtime.py` (Phase 64.0) *does* import
...
**Resolution**: one new file, `voice/conversation_bridge.py`, is the
second, narrowly-scoped composition-root exception in this codebase —
the same justification `ai/intelligence_runtime.py`'s own docstring
...
layer"). `voice/conversation_bridge.py`'s one function,
...
(which `voice/profiles.py` catalog entry — Senior/Seniorita/Narrator),
...
Confirmed: `ai.persona.persona_registry.SENIOR_TRADING_AI.name ==
...
existing `VoiceProfile` by name; nothing about `ai.persona.Persona`
...
- `voice/conversation_bridge.py`: the one new file permitted to import
  `ai.conversation.conversation_engine.ConversationEngine`,
  `voice.stt.manager.STTManager`, `voice.intents.detector`,
  `voice.runtime.VoiceRuntime`, `voice.session.models.VoiceSession` —
...
No Constitution conflict. `voice/conversation_bridge.py` is
consciously modeled on `ai/intelligence_runtime.py`'s own precedent
...
Trading, `ai/intelligence_runtime.py` for the deterministic 8-layer
