...
command groups under `telegram/owner/`, each following the same
Handler → Service → Repository flow as every other Telegram command
(Constitution Article 4), gated by `telegram/owner/owner_roles.py`.

## Owner Telegram Control Center — current shape

`telegram/owner/` today (19 files), grouped into the sections the
...
| **Emergency** | `emergency_commands.py`, backed by `core/emergency/` and `database/emergency_repository.py` |
...
Owner (Telegram) → command_router.py → permissions.py (owner_roles.py check)
    → telegram/owner/<domain>_commands.py handler
    → telegram/*_service.py or ai/runtime/ai_service.py
    → database/*_repository.py
```

No `telegram/owner/*.py` file queries the database directly — the
...
`telegram/owner/*.py` file for any of these is only justified once a
