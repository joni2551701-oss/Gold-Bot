...
| Session/time logic (`data_layer/live_data/session_filter.py` vs `context/session.py`) | No (verified in TASK 3) | Different questions (wall-clock gate vs per-candle classification). | Leave unchanged; `session_filter.py`'s dead-code status is the real issue (TASK 3), not duplication. |
