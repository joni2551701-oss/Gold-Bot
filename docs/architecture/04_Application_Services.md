...
| Service | Status |
|---|---|
| Signal Service | Not a standalone service — signal output is currently formatted and delivered directly by `platform_layer/telegram/signal_formatter.py`/`notifier.py` inside the pipeline, not through a separate service boundary. |
...
| Notification Service | Real, beyond `platform_layer/telegram/notifier.py` — `platform_layer/telegram/notification_service.py`, `platform_layer/telegram/owner/runtime_notifications.py`, plus a `broadcast/` package for multi-channel delivery (contract-only, Phase 63.0). |
| Replay Service | Real as foundation — `backtesting_layer/replay_engine/` (module 8), plus `platform_layer/telegram/owner/replay_commands.py`. Not wired into the live pipeline. |
