...
| Application Services boundary | Mostly missing (`04_Application_Services.md`) | A real service boundary between Core and Telegram, so a future Web/Mobile client doesn't need its own copy of `platform_layer/telegram/signal_formatter.py`'s logic | Medium (blocks Platform Layer expansion, not urgent while Telegram is the only platform) | High once a second platform is planned | Low — additive, no existing consumer needs to change |
