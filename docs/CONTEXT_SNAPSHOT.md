...
| `Session.LONDON.value == "LONDON"` (and every other session) | `context_layer/session/session.py` (Phase A6) | `SessionInfo.current_session` — the latest `SessionEvent.session.value`, same pattern `core_layer/features/feature_engine.py` (Phase A10) already used. |
