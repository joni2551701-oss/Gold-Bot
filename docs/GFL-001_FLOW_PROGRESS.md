...
Flow'ning tegishli vazifasi boshqa yo'l bilan (masalan boshqa Flow
orqali) jonli production'da bajarilmoqda va bu real kod audit bilan
tasdiqlangan; Flow'ning o'z nomlagan komponenti esa to'liq qurilgan,
test qilingan Foundation bo'lib qoladi — real, sun'iy bo'lmagan
Consumer yo'qligi sababli ataylab Production'ga ulanmagan (Director
Decision talab qiladi). Completed emas — lekin bu xato yoki
tugallanmaganlik emas, real holatni aks ettiradi.
...
|------|------|--------------------|--------|----------|-------|------|
| FLOW-001 | System Bootstrap / Configuration | Foundation Layer | 🟩 | 100% | Worker | Yakunlandi -- 2026-08-05 (PHASE-02 Flow-by-Flow Production Development). Qisqa Audit: butun zanjir allaqachon real production'da mavjud -- Producer `main.py` (`from config import Config` + GoldBot -> TradingPipeline); Input env vars + `core_layer.secrets.Secrets` (+ `.env` loader, CI GitHub Secrets); Processing `config.py` (`Config`, `build_settings()`, `get_settings()`, `Settings`); Output `Settings` singleton; Consumer Data Layer -- `provider_manager` `config.get_settings().providers` o'qiydi (FLOW-002). Reuse First: yangi kod kerak emas. `python main.py` orqali production'da isbotlangan; `tests/configuration/*` (test_central_config/environment/secret_names). 5432 test PASS. |
...
| FLOW-016 | Chart Service | GoldBot > Chart Service | 🟩 | 100% | Worker | Yakunlandi -- 2026-08-05 (PHASE-02 Production Development + Director Decision "Architecture Correction"). Chart Service Production Foundation (Core Infrastructure) real kod bilan qurildi. Audit ziddiyat topdi: dastlabki "10 alohida paket" reja canonical blueprint bilan zid edi (8 ta ortiqcha top-level paket, `Chart_Core`/`Chart_Data`/`Chart_API` bilan ustma-ust). Director "Architecture Correction"ga ruxsat berdi. Correction: 10 komponent mavjud 5 canonical subpaketga joylashtirildi (yangi ortiqcha paketsiz) -- `chart_data` (models/request/response/cache), `chart_renderer` (renderer), `chart_core` (pipeline/engine), `chart_api` (service/api/events). Zanjir ishlaydi: Market Memory port -> Pipeline -> Engine -> Renderer -> Service -> API -> Platform; `python main.py` OK. Scope: Core Infrastructure only (real render/Drawing/Indicators/Replay keyingi Flow'larda shu Foundation ustiga). To'liq audit: `docs/FLOW_016_CHART_ARCHITECTURE_CORRECTION.md`. Testlar: `tests/chart_layer/` (31 test PASS). |
| FLOW-017 | Personal AI Core | GoldBot > Personal AI Core | 🟩 | 100% | Worker | Yakunlandi -- 2026-08-05 (PHASE-02 FLOW-017 Production Wiring + Memory System, Director Order). Personal AI mahsuloti uchun **jonli user-facing Consumer** ulandi: `/ask <savol>` (OWNER-only) -> `command_router` -> `handlers.ask_handler` -> `ai_layer/personal_ai/interaction_manager/memory_first_query.answer_question`. **Memory-First** oqim (Director Memory Rule STEP-1..6): STEP-1 `MemoryRuntime.recall` -> STEP-2 hit bo'lsa API YO'Q -> STEP-3 miss bo'lsa `ConversationEngine.ask` (External AI API) -> STEP-5 javob `MemoryEntry(scope=KNOWLEDGE_REFERENCE, type=LONG_TERM, value=MemoryAnswer)` sifatida saqlanadi -> STEP-6 javob. Memory Contract (`MemoryAnswer`): Question/Answer/Topic/Tags/Timestamp/Source/Confidence/Version. Yangi LLM/Prompt/Engine/Service/Memory System yozilmadi -- faqat mavjud modullar reuse+wiring (composition root). API **hech qachon** Memory tekshirilmasdan chaqirilmaydi (test bilan isbotlangan). Owner-Mode gate (`is_personal_ai_enabled_for`, `enable_personal_ai` default OFF) saqlangan -- default toggle o'zgartirilmadi (access-control, alohida Director qarori). To'liq Director Review: `docs/FLOW_017_PERSONAL_AI_PRODUCTION.md`. Testlar: `tests/ai/personal_ai/test_memory_first_query.py` (12 test PASS: Unit/Integration/E2E). |
| FLOW-018 | Backtesting Engine | GoldBot > Backtesting Engine | 🟩 | 100% | Worker | Yakunlandi -- 2026-08-06 (PHASE-02 FLOW-018 Production Wiring + Tool-First Foundation, Director Order). Backtesting Engine uchun **jonli user-facing Consumer** ulandi: `/backtest <SYMBOL> <TF> <START> <END>` (OWNER-only) -> `command_router` -> `handlers.backtest_handler` -> `backtesting_layer/backtest_service/backtest_service.BacktestService` -> `BacktestEngine.run()` -> `format_backtest_report`. Director Production Pipeline'ning **Backtesting Service** qatlami mavjud bo'sh `backtest_service` skeleton paketida real implement qilindi (Reuse Rule — yangi top-level paket emas). **Tool-First:** Backtesting Tool Historical Data'ni **DB**'dan (`RawCandleRepository`/`ReplayEngine`) o'qiydi — butun yo'lda tashqi API yo'q. Yangi Engine/Statistics/Replay/Trading logic yozilmadi (reuse+wiring). `backtest_commands.backtest_run()` endi servisga delegatsiya qiladi (duplicate logic yo'q). To'liq Director Review: `docs/FLOW_018_BACKTESTING_PRODUCTION.md`. Testlar: `tests/backtesting/test_backtest_service.py` (15 test PASS: Unit/Integration/E2E). |
| FLOW-019 | Application Services | Application Services | 🟩 | 100% | Worker | **Completed (PHASE-02 Final Sprint, 2026-08-07, Director scope korreksiyasi + DRQ-001 Option B APPROVED).** Director korreksiyasi: FLOW-019'ning maqsadi `PlatformService`ni Production qilish emas -- Application/Service Layer'ni Telegram orqali to'liq Production holatiga olib chiqish. Bu maqsad allaqachon bajarilgan: `platform_layer/telegram/*_service.py` (9 ta servis -- `UserService`, `SubscriptionService`, `SignalService`, `AdminService`, va h.k.) jonli, Handler'lar orqali chaqiriladi, test qilingan (`tests/telegram/*`, 14+ fayl), FLOW-001/FLOW-020 orqali production'da isbotlangan. `platform_service` (PlatformRegistry/MenuRegistry/NavigationCore/PlatformAdapterBase/ModuleCapabilityRegistry) FLOW-019'ning nishoni emas -- bu kelajakdagi ko'p-platformali (Mobile/Desktop/Web) abstraction, Foundation'da ataylab saqlanadi va shu platformalar qo'shilganda tabiiy ravishda ishlatiladi (Sun'iy Consumer yaratish hozir taqiqlangan). Batafsil: `docs/FLOW_019_APPLICATION_SERVICES_FOUNDATION.md`. |
...
| FLOW-025 | Web | Platform Layer | 🟦 | 0% | Worker | Audit qilindi -- 2026-08-05. `platform_layer/web_api/` faqat 13-qatorli Foundation Freeze skeleton, real UI kod yo'q. Sub-Status: Blueprint (Development uchun tayyor emas). |
