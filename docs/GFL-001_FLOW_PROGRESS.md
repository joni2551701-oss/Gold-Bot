...
| FLOW-018 | Backtesting Engine | GoldBot > Backtesting Engine | 🟨 | 70% | Worker | **Partial (Production re-audit, 2026-08-05).** `BacktestEngine`/`BacktestResult`/statistics real + test qilingan; statistics modullari `ai_layer` analytics tomonidan iste'mol qilinadi. Ammo `BacktestEngine.run()` uchun jonli Consumer YO'Q -- `backtest_commands.py` orphaned (`OWNER_COMMANDS`da yo'q, `handlers.py`da yo'q, workflow yo'q). Eski 100% docs-only edi. Batafsil: `docs/FLOW_017_025_PRODUCTION_REAUDIT.md`. Reja: Sprint P2 (jonli `/backtest` komandasini ulash). |
