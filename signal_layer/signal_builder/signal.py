...
already-existing identity/validation helpers into one import surface.
    ...
    from signal_layer.signal_builder.signal import CanonicalSignal, generate_signal_id

`CanonicalSignal` IS `signal_layer.signal_builder.schema.SignalSchema` — same frozen dataclass,
...
the STEP-08 pipeline's surrounding structures (signal_layer/signal_scoring/quality.py's
SignalStrength, signal_layer/signal_builder/enricher.py's SignalEnrichment, signal_layer/signal_formatter/formatter.py's
...
from signal_layer.signal_builder.schema import (  # re-export: single canonical import surface
