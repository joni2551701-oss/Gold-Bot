...
GoldBot package. `adapter.py` imports `signal_layer.signal_builder.schema` (same package)
plus, `TYPE_CHECKING`-only, `signal_layer.signal_builder.models`/`signal_layer.signal_scoring.signal_quality`/
`decision_layer.decision_engine.models` — no runtime dependency on `decision/`, and no
...
above, taking a setup-layer `strategy_layer.strategy_engine.result.StrategyResult` and
