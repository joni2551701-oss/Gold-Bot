...
`ai_layer.ai_engine.providers.provider_manager.ProviderManager`'s live status,
optionally `ai_layer.ai_engine.capabilities.capability_manager.CapabilityManager`'s
live enabled/disabled state, and -- as of Phase 61.1 -- optionally
`ai_layer.ai_engine.providers.provider_capabilities.supports()` and
`ai_layer.ai_engine.providers.provider_health.ProviderHealthTracker`'s live health --
...
-- a read-only view over `ai_layer.ai_service.audit.provider_stats.compute_provider_stats()`,
...
from ai_layer.ai_service.audit.provider_stats import ProviderStats, compute_provider_stats
...
from ai_layer.ai_engine.providers.provider_capabilities import supports
...
from ai_layer.ai_coordinator.routing_result import RoutingResult
...
class AIRouter:
    ...
    isolation), matching `ai_layer.ai_engine.context.context_builder`'s own
