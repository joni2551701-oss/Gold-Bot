...
Fourth non-placeholder `BaseAIProvider` implementation, following
`ai/providers/gemini_provider.py`'s exact pattern (Phase 61.2 TASK 3).
`core/secrets.py` already carries `GROK_API_KEY` as an optional secret
(Phase 61.2 TASK 2) — no secrets change needed for this task.

Real HTTP call via `requests` (no new SDK dependency) directly against
xAI's OpenAI-compatible chat-completions REST API. The API key travels
only in the `Authorization` request header, never in the URL/query
string.
"""
...
logger = setup_logger("GrokProvider")
...
class GrokProvider(BaseAIProvider):
    """`session`/`secrets` are both injectable so a test never performs a real network call or needs a real environment variable set -- see `tests/ai/providers/test_grok_provider.py`."""

    def __init__(self, session=None, secrets: Optional[Secrets] = None) -> None:
        self._session = session or requests
        self._secrets = secrets or Secrets()
        try:
            self._api_key = self._secrets.GROK_API_KEY
        except Exception:
            self._api_key = None
    ...
    def _generate(self, prompt: str) -> ProviderResult:
        """The one real network call every text capability routes through. Raises `ai_layer.ai_engine.providers.runtime_errors.ProviderRuntimeError` (never a raw `requests` exception, never the API key) on any failure."""
        if self._api_key is None:
            raise ProviderUnavailableError(self.name, "GROK_API_KEY not configured")

        headers = {"Authorization": f"Bearer {self._api_key}", "Content-Type": "application/json"}
        payload = {"model": _GROK_MODEL, "messages": [{"role": "user", "content": prompt}]}

        try:
            response = self._session.post(
                _GROK_ENDPOINT, headers=headers, json=payload, timeout=_REQUEST_TIMEOUT_SECONDS,
            )
    ...
    def vision(self, prompt: str, image_ref: Optional[str] = None) -> ProviderResult:
        raise NotImplementedError("GrokProvider.vision() -- real multimodal support not implemented this phase")

    def image(self, prompt: str) -> ProviderResult:
        raise NotImplementedError("GrokProvider.image() -- real image generation not implemented this phase")

    def voice(self, prompt: str) -> ProviderResult:
        raise NotImplementedError("GrokProvider.voice() -- real voice support not implemented this phase")
