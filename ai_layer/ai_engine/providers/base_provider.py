...
call for (not just `evaluate()`/analysis, unlike `ai_layer/ai_service/interfaces.py`'s
...
this is a genuinely new package, not a duplicate of `ai_layer/ai_engine/ai_analyzer.py`
(the live, single-purpose production analyzer) or `ai_layer/ai_engine/ai_prompt.py`
(a Gemini-specific prompt builder).

Same advisory-only boundary as `ai_layer/ai_service/interfaces.py`'s
...
`ai_layer.ai_engine.providers.provider_capabilities.capabilities_of()` directly rather
...
class BaseAIProvider(ABC):
    ...
    def capabilities(self) -> FrozenSet[Any]:
        """Reuses `ai_layer.ai_engine.providers.provider_capabilities.capabilities_of(self.name)` directly -- no per-provider override needed unless a provider's real, implemented support differs from the declared matrix (see `ai/providers/gemini_provider.py`'s own docstring for that exact case)."""
        from ai_layer.ai_engine.providers.provider_capabilities import capabilities_of
