...
Third non-placeholder `BaseAIProvider` implementation, following
`ai/providers/gemini_provider.py`'s exact pattern (Phase 61.2 TASK 3).
`core/secrets.py` already carries `CLAUDE_API_KEY` as an optional
secret (Phase 61.2 TASK 2) — no secrets change needed for this task.

Real HTTP call via `requests` (no new SDK dependency) directly against
Anthropic's Messages REST API. The API key travels only in the
`x-api-key` request header, never in the URL/query string.
"""
...
logger = setup_logger("ClaudeProvider")
...
class ClaudeProvider(BaseAIProvider):
    """`session`/`secrets` are both injectable so a test never performs a real network call or needs a real environment variable set -- see `tests/ai/providers/test_claude_provider.py`."""

    def __init__(self, session=None, secrets: Optional[Secrets] = None) -> None:
        self._session = session or requests
        self._secrets = secrets or Secrets()
        try:
            self._api_key = self._secrets.CLAUDE_API_KEY
        except Exception:
            self._api_key = None
    ...
    def _generate(self, prompt: str) -> ProviderResult:
        ...
        if self._api_key is None:
            raise ProviderUnavailableError(self.name, "CLAUDE_API_KEY not configured")
        ...
        try:
            response = self._session.post(
                _CLAUDE_ENDPOINT, headers=headers, json=payload, timeout=_REQUEST_TIMEOUT_SECONDS,
            )
    ...
    def vision(self, prompt: str, image_ref: Optional[str] = None) -> ProviderResult:
        raise NotImplementedError("ClaudeProvider.vision() -- real multimodal support not implemented this phase")

    def image(self, prompt: str) -> ProviderResult:
        raise NotImplementedError("ClaudeProvider.image() -- real image generation not implemented this phase")

    def voice(self, prompt: str) -> ProviderResult:
        raise NotImplementedError("ClaudeProvider.voice() -- real voice support not implemented this phase")
