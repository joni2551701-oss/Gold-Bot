...
`ai_layer.ai_engine.providers.provider_health.ProviderHealthTracker` -- the single
source of truth `ai_layer.ai_coordinator.router.AIRouter.route()` already reads via
...
State mapping into `HealthStatus` (`ai_layer.ai_engine.providers.provider_status`):
