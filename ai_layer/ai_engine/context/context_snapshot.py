...
class AIContext:
    ...
    the same way `ai_layer/ai_service/interfaces.py`'s `AIAnalyzerInterface.evaluate()`
    ...
    snapshot_id: None until `build_ai_context()` sets it -- a plain
        ...
        through the canonical path; `ai_layer.ai_engine.cache.cache_policy.build_cache_key_from_context()`
