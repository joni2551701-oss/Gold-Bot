...
- `reasoning_result_to_explanation_fields()` -- Explanation is
  *downstream* of Reasoning, so this function returns a plain `dict`
  ...
  without importing `ai_layer.explanation_ai` at all. A future caller (not this
...
from ai_layer.ai_engine.reasoning.models import ReasoningResult, ReasoningStep
