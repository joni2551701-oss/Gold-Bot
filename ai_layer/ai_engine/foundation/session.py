...
service" posture the existing `ai_layer.ai_service.session.SessionManager` uses. This is
...
import of `ai_layer.ai_service.session` (that package transitively pulls `core_layer.logger.logger`,
which the Foundation->Core prohibition forbids).

Depends only on stdlib and `ai_layer.ai_engine.foundation.context`.
"""
...
from ai_layer.ai_engine.foundation.context import FoundationContext
