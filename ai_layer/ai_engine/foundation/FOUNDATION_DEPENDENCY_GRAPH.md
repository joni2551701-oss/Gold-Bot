...
`ai_layer.ai_engine.foundation.* → ai.foundation.*` import; there are **no** edges to
...
- **Other `ai/` subpackages:** none. The subpackage-level cycle
  detector reports `ai_layer.ai_engine.foundation` with zero cross-subpackage edges.
- **Core / Platform / Media / Telegram / Database:** none — not even
  ...
  stdlib and `ai_layer.ai_engine.foundation.*` imports.
