...
`ai_layer.personal_ai.senior.coaching_runtime.CoachingRuntime.create()` accepts -- it
...
field (unlike `ai_layer.knowledge_ai.learning_engine.models.LearningRecord.topic`, which
`ai_layer.personal_ai.senior.learning_adapter.py` can relay directly), so choosing one
...
follows. Mirrors `ai_layer.personal_ai.senior.journal_adapter.journal_entry_to_coaching_input()`'s
own "topic deliberately omitted" precedent exactly.

Mirrors `ai_layer.personal_ai.senior.journal_adapter.py`'s own shape exactly: it reads
its *source* package's model (`ai_layer.ai_engine.performance.models.PerformanceRecord`,
already local to this package) and returns a plain, untyped `Dict` of
`CoachingRuntime.create()` kwargs -- it never imports `ai_layer.personal_ai.senior.models`
or `ai_layer.personal_ai.senior.coaching_runtime` at all (no import is needed to
