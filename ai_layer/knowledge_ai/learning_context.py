...
`ai_layer/ai_service/interfaces.py` already documents applies here too -- a future AI
...
Reuses `ai_layer.knowledge_ai.learning_loop.pattern_detector.detect_patterns()`/
`filter_high_success_patterns()`/`format_pattern_insight()` directly
for the `successful_patterns` list, and
`backtesting_layer.statistics.strategy_report.compute_win_rate()` directly for
...
- `confidence`: one `"<condition>: <LEVEL>"` line per pattern, via
  `ai_layer.knowledge_ai.learning_loop.confidence.compute_pattern_confidence()` (TASK 5) and
  `ai_layer.knowledge_ai.learning_loop.pattern_detector.group_records_for_patterns()` (the same
...
- `regimes`: caller-supplied, pre-formatted regime observation
  strings (e.g. from `ai_layer.knowledge_ai.learning_loop.regime_memory`, TASK 7) -- accepted as
  a plain `Sequence[str]` rather than importing `ai_layer.knowledge_ai.learning_loop.regime_memory`
...
from ai_layer.knowledge_ai.learning_loop.confidence import compute_pattern_confidence
...
class LearningContext:
    ...
    confidence (Phase 60.7): one `"<condition>: <LEVEL>"` line per
        ...
        `ai_layer.knowledge_ai.learning_loop.confidence.compute_pattern_confidence()`.
    """
...
def build_learning_context(
    ...
    `backtesting_layer.statistics.learning_report.build_learning_report()` call) can pass
    it directly to avoid recomputing. `regimes` is relayed as-is (an
    already-formatted sequence, e.g. from a future
    `ai_layer.knowledge_ai.learning_loop.regime_memory` caller) -- this module never fetches or
