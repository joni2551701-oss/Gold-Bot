...
ground: `backtesting_layer/statistics/signal_performance.py`'s `SignalPerformance` shares
...
class LearningRecord:
    ...
    strategy_name: matches `SignalSchema.strategy_name`'s own value —
        ...
        `backtesting_layer/statistics/signal_performance.py` already uses.
    market_phase/session/timeframe: the same three context dimensions
        `backtesting_layer/statistics/signal_performance.py`/`context_report.py` already
    ...
    result: one of `trade_monitoring_layer.paper_trading.paper_trade.ALLOWED_PAPER_TRADE_RESULTS`
        ...
        vocabulary `backtesting_layer.statistics.signal_performance.SignalPerformance.result`
        already uses.
    r_multiple: the realized risk-multiple — same value
        `backtesting_layer.statistics.signal_performance.compute_r_multiple()` already
    ...
    failure_type: a short, free-text classification of *why* a losing
        ...
        `ai_layer.knowledge_ai.knowledge_base.journal.failure_analysis.FailureAnalysisEntry.reason`
        already established.
    ...
    confidence_score (Phase 60.7): whatever single confidence measure
        ...
        to be confused with `ai_layer.knowledge_ai.learning_loop.confidence.compute_pattern_confidence()`
    ...
    sample_size (Phase 60.7): an honest hook, always None unless a
        ...
        live on `ai_layer.knowledge_ai.learning_loop.pattern_detector.PatternInsight.occurrences`
