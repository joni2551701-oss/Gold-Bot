...
`ai_layer.knowledge_ai.learning_loop.models.LearningRecord`, via
`ai_layer.knowledge_ai.learning_loop.outcome_analyzer.analyze_trade_result()` (reused directly,
...
from ai_layer.knowledge_ai.learning_loop.models import LearningRecord, create_learning_record
from ai_layer.knowledge_ai.learning_loop.outcome_analyzer import analyze_trade_result
...
def _volatility_state(context: Optional['ContextSnapshot']) -> Optional[str]:
    ...
    `ai_layer.knowledge_ai.learning_loop.models.LearningRecord.volatility_state`'s own docstring.
    """
    if context is None:
        return None
...
def build_learning_record_from_trade(
    ...
    (`" + "`-separated, matching `ai_layer.knowledge_ai.learning_loop.pattern_detector`'s own
