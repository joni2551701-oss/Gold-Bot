...
- `conversation_context_to_explanation_fields()` -- Explanation is
  *downstream* of Conversation, so this function returns a plain
  ...
  fields without importing `ai_layer.explanation_ai` at all.
"""

from typing import Dict

from ai_layer.personal_ai.interaction_manager.models import ConversationContext
from ai_layer.knowledge_ai.memory_manager.models import MemoryEntry
from ai_layer.ai_engine.reasoning.models import ReasoningResult
from ai_layer.knowledge_ai.knowledge_base.models import KnowledgeEntry
...
def conversation_context_to_explanation_fields(context: ConversationContext) -> Dict[str, str]:
    ...
    `ai_layer.explanation_ai.explanation_input.ExplanationInput`'s own fields --
