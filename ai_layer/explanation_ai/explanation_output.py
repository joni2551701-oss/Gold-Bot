...
class ExplanationOutput:
    ...
    metadata: free-form, provider/generation-specific detail, same
        convention `ai_layer.ai_engine.providers.base_provider.ProviderResult.metadata`
        already uses.
    market_context: (Phase 63.1) plain-language market-condition
        sentence (e.g. "Gold remains bullish on H1.") -- distinct from
        `ai_layer.ai_service.interfaces.MarketContext`, which is a data object; this is
    ...
    persona: (Phase 63.1) the `ai_layer.personal_ai.persona_manager.persona.Persona.name` this
