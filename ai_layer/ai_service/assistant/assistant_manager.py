...
`ai_layer.personal_ai.persona_manager.persona_manager.PersonaManager`,
`ai_layer.ai_service.session.session_manager.SessionManager`, or
`ai_layer.voice_ai.session.manager.VoiceSessionManager`, and why runtime
...
Owner-gated via `ai_layer.ai_service.assistant.access.is_personal_ai_enabled_for()`
...
`ai_layer.ai_engine.intelligence_runtime` -- per Phase 65.4's own Director Note 3
...
`ai_layer/ai_service/assistant/runtime_adapter.py`.
