...
`AssistantIdentity` is deliberately NOT `ai_layer.personal_ai.persona_manager.persona.Persona`:
...
conventional -- the same posture `ai_layer/voice_ai/profiles.py`'s own
...
class AssistantIdentity:
    ...
    name: the identity's stable key (e.g. "Senior", "Seniorita") --
        coincidentally matches `ai_layer.voice_ai.profiles.SENIOR_VOICE.name`/
    ...
    default_voice: a `ai_layer.voice_ai.profiles` profile *name* string this
        identity resolves to when no explicit voice override is
        selected ("Voice: Auto" per TASK 8) -- a free-text pointer,
        never a `ai_layer.voice_ai.models.VoiceProfile` object.
