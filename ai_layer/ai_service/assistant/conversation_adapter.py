...
calls `ai_layer.voice_ai.session.manager.VoiceSessionManager.create_session(**...)`
or `ai_layer.personal_ai.interaction_manager.conversation_engine.ConversationEngine.start_session(**...)`
...
def assistant_to_voice_session_params(profile: AssistantProfile, identity: AssistantIdentity) -> Dict[str, str]:
    """
    Shaped to match `ai_layer.voice_ai.session.manager.VoiceSessionManager.create_session()`'s
...
def assistant_to_conversation_params(profile: AssistantProfile) -> Dict[str, str]:
    """Shaped to match `ai_layer.personal_ai.interaction_manager.conversation_engine.ConversationEngine.start_session()`'s existing `(telegram_id)` parameter -- `user_id` is reused as the telegram_id pointer, never a new identifier scheme."""
...
def assistant_memory_scope_key(profile: AssistantProfile, sub_key: Optional[str] = None) -> str:
    ...
    `ai_layer.personal_ai.interaction_manager.conversation_adapters.memory_key_from_entry()`
    already establishes -- without importing
    `ai_layer.knowledge_ai.memory_manager.models.MemoryScope` (the literal `"USER_PREFERENCE"`
