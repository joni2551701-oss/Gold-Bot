...
`ai/runtime/event_bus.py` to this neutral top-level `ai_layer/ai_service/event_bus.py`.
...
alarming condition). `ai_layer.ai_engine.runtime.runtime_manager.RuntimeManager.
...
not `AI_RUNTIME_STARTED`). `ai_layer.ai_engine.runtime.ai_service.AIService.ask()`
...
class EventBus:
    ...
    (`ai_layer.ai_service.session.session_manager.SessionManager`,
    `ai_layer.ai_engine.providers.provider_health.ProviderHealthTracker`). `publish()`
