...
- `compute_requests_per_minute()` -- a pure function over
  `ai_layer.ai_service.audit.request_log.AIRequestLogEntry.created_at`, the same
...
- `RuntimeMetrics`/`RuntimeMetricsCollector` -- cache hit/miss,
  ...
  `ai_layer.ai_service.event_bus.EventBus` (Phase 61.6 TASK 5) and accumulates
...
from ai_layer.ai_service.audit.response_log import AIResponseLogEntry
