...
`ai_layer.ai_service.content.broadcast_output.BroadcastReadyContent` contract
...
def explanation_to_broadcast_ready(explanation: ExplanationOutput) -> BroadcastReadyContent:
    """Never raises: every `ExplanationOutput` has a non-empty `title`/`body` by construction, so this never needs to reject one the way `ai_layer.ai_service.content.broadcast_output.prepare_broadcast()` rejects a non-accepted `ContentResult`."""
