...
reads an existing `ai_layer.ai_engine.portfolio.models.PortfolioRecord` (Phase 66.7,
LOCKed) type-only and produces the plain keyword arguments
`ai_layer.fundamental_ai.research_runtime.ResearchRuntime.create()` accepts -- it
...
`ai_layer.fundamental_ai.performance_adapter.py`'s own docstring already applies.
...
`ai_layer.ai_engine.portfolio.models` -- `models.py`, `access.py`, `research_runtime.py`,
