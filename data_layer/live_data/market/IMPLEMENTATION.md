...
- the latest price (a `MarketPrice`, or read from a
  `data_layer.live_data.stream.current_price.CurrentPrice` by symbol), plus optional recent candles.
...
- **`session_state.py`** — `SessionState` from context session +
  `data_layer.live_data.stream.stream_mode.is_weekend` (agrees with the stream weekend clock).
