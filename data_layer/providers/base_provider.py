...
NAMING NOTE -- read before using this module: data_layer/providers/twelve_data_client.py
already defines Candle (timestamp, open, high, low, close -- no
symbol/timeframe/volume of its own, the real type
data_layer/live_data/market_data.py's MarketDataNormalizer and the entire live
...
Candle, and data_layer/providers/twelve_data_client.py is untouched by this phase; a
...
class MarketCandle:
    ...
    same. See data_layer/normalization/candle_normalizer.py's
...
class DataProvider(ABC):
    """
    The truly universal contract every data source in data_layer/providers/
...
class MarketDataProvider(DataProvider):
    ...
    def get_candles(self, symbol: str, timeframe: str, limit: int) -> List[MarketCandle]:
        ...
        matching data_layer/providers/twelve_data_client.py's own fetch_candles()
