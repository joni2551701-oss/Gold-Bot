...
**Audit finding**: `data_layer/providers/base_provider.py`'s `MarketCandle`
...
- Does not change `data_layer/providers/twelve_data_client.py`'s own
  `_format_symbol()`/`INTERVAL_MAP`, or `data_layer/providers/binance_provider.py`'s
  ...
  established.
- Does not wire into `core/pipeline.py` or `data_layer/live_data/market_data.py`.

## Dependencies
`symbol_mapper.py`/`timeframe_mapper.py` import only the standard
library. `candle_normalizer.py` imports
`data_layer.providers.base_provider.MarketCandle`. None imports `context/`,
