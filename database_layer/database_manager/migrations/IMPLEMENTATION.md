...
`ALTER TABLE ADD COLUMN` calls in `database_layer/database_manager/models.py`, run on every
...
2. **Idempotent, always.** Every migration must be safe to run twice
   ...
   double-effect. `database_layer/database_manager/models.py`'s existing
...
- No migration in this project may be force-applied past a failure.
  ...
  `database_layer/database_manager/models.py`'s existing pattern: every `ALTER TABLE`/
  `CREATE INDEX` call is wrapped in `try/except sqlite3.Error` that
  logs and re-raises, never silently continues past a failed
  statement).

## Relationship to `database_layer/database_manager/models.py`

`database_layer/database_manager/models.py` remains the source of truth for the current
