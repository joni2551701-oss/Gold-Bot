...
database_layer/trade_repository/emergency_models.py's own shape/naming split: the DB-row
...
Append-only by design (see database_layer/trade_repository/risk_decision_repository.py):
...
class RiskDecisionEntry:
    ...
    symbol: the traded symbol, or "UNKNOWN" when RiskManager.evaluate()
        ...
        backtesting_layer/backtest_engine/backtest_engine.py call sites, both untouched this
