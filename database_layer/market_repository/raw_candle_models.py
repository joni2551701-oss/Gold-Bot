...
from (see database_layer/market_repository/raw_candle_repository.py's UNIQUE(symbol,
...
def create_raw_candle(
    ...
    """Same str(uuid.uuid4())-free, datetime.now(timezone.utc)-stamped convention as database_layer/trade_repository/signal_record.py's create_signal_record() -- created_at is when this record was built, not when the candle's own timestamp occurred."""
    return RawCandle(
        symbol=symbol,
        timeframe=timeframe,
        timestamp=timestamp,
...
def from_market_candle(candle: 'MarketCandle', provider: Optional[str] = None) -> RawCandle:
    ...
    capture_market_data_snapshot() and database_layer/market_repository/market_snapshot_models.py's
